...
class _TimesheetPdfViewerState extends State<TimesheetPdfViewer> {
  ...
  Future<void> _loadPDF() async {
    try {
      ...
      print('📋 [TimesheetPdfViewer] Response length: ${response.bodyBytes.length}');
  ...
  /// ✅ Save PDF to public Downloads folder (accessible by user)
  Future<void> _saveToDownloads(String sourcePath) async {
    try {
      final timestamp = DateTime.now().millisecondsSinceEpoch;
      String fileName = 'timesheet_${widget.employeeName}_${_formatPeriod(widget.period)}_$timestamp.pdf'
          .replaceAll(' ', '_')
          .replaceAll('/', '-')
          .replaceAll(':', '')
          .replaceAll('\\', '');
      ...
      if (Platform.isAndroid) {
        // ✅ FIXED: Try to use public Downloads folder first
        List<String> publicDownloadPaths = [
          '/storage/emulated/0/Download',
          '/storage/emulated/0/Downloads',
        ];

        bool savedToPublic = false;
        for (String path in publicDownloadPaths) {
          try {
            ...
            if (await testDir.exists()) {
              ...
              downloadPath = path;
              savedToPublic = true;
              print('📁 Using public Downloads: $downloadPath');
              break;
            }
          } catch (e) {
        ...
        if (!savedToPublic) {
          ...
          if (saveDir == null) {
            throw Exception('Cannot access any storage');
          }
          downloadPath = '${saveDir.path}/Downloads';
          await Directory(downloadPath).create(recursive: true);
          print('📁 Using app external Downloads: $downloadPath');
        }
      } else {
        // iOS: Use documents directory
        final saveDir = await getApplicationDocumentsDirectory();
        downloadPath = saveDir.path;
        print('📁 Using documents directory: $downloadPath');
      }
      ...
      // ✅ Show appropriate success message based on save location
      String locationMessage;
      if (Platform.isAndroid) {
        if (downloadPath.contains('/storage/emulated/0/Download')) {
          locationMessage = 'Saved to: Downloads folder\n$fileName';
        } else {
          locationMessage = 'Saved to: Android/data/.../Downloads/\n$fileName';
        }
      } else {
    ...
    if (localPath != null) {
      try {
        await Share.shareXFiles(
          [XFile(localPath!)],
          text: 'Timesheet - ${widget.employeeName} (${_formatPeriod(widget.period)})',
        );
      } catch (e) {
